...
public class LoginWithGoogle extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Staffs staff = staffsDAO.getStaffByEmail(user.getEmail());
            ...
            if (userInDB != null) {
                ...
                customer.setPassword(PasswordUtils.hashPassword(password));
                customer.setRole(Customers.Role.USER);
                customer.setStatus(Customers.Status.ACTIVE);
                customer.setGender(Customers.Gender.MALE);
                udao.registerUser(customer);

                // Gửi mật khẩu đến email người dùng
                MailService.sendMail(customer.getEmail(), "Password!", "Your password is: " + password);
    ...
    public static Users getUserInfo(final String accessToken) throws ClientProtocolException, IOException {
        ...
        JsonObject jobj = new Gson().fromJson(response, JsonObject.class);
        Users user = new Users(jobj.get("name").getAsString(), jobj.get("email").getAsString());
        return user;
    }
