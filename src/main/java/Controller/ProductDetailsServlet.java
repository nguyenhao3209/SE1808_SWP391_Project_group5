...
// Lấy thông tin sản phẩm từ database
        ...
        ArrayList<Specifications> specifications = productDAO.getSpecificationsByProductId(productId);
...
// Lấy danh sách đánh giá cho sản phẩm
        ...
        List<Feedback> reviews = reviewsDao.getReviewsByProductId(product.getProductID());
        session.setAttribute("reviews", reviews);

        Customers user = (Customers) session.getAttribute("user");
        String customerId = user == null ? null : user.getCustomerId();
        boolean isBought = false;

        if (customerId != null) {
            isBought = reviewsDao.isBought(customerId, productId);
